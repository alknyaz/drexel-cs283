...
int open_db(char *dbFile, bool should_truncate){
    ...
    ftruncate(fd, (MAX_STD_ID + 1) * sizeof(student_t));
...
int get_student(int fd, int id, student_t *s){
    if (lseek(fd, id * STUDENT_RECORD_SIZE, SEEK_SET) == -1) return ERR_DB_FILE;

    if (read(fd, s, STUDENT_RECORD_SIZE) == -1) return ERR_DB_FILE;

    if (s->id == EMPTY_STUDENT_RECORD.id) return SRCH_NOT_FOUND;
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    if (write(fd, &s, STUDENT_RECORD_SIZE) == -1) {
...
int count_db_records(int fd){
    if (lseek(fd, MIN_STD_ID * sizeof(student_t), SEEK_SET) == -1) {
      printf(M_ERR_DB_READ);
      return ERR_DB_FILE;
    }
    ...
    int err = 0;
    while ((err = read(fd, &s, sizeof(student_t))) > 0 && s.id == EMPTY_STUDENT_RECORD.id);
    ...
    if (s.id == EMPTY_STUDENT_RECORD.id) {
      printf(M_DB_EMPTY);
      return NO_ERROR;
    }
    ...
    while ((err = read(fd, &s, sizeof(student_t))) > 0)
      count += s.id != EMPTY_STUDENT_RECORD.id;
...
int print_db(int fd){
    ...
    do {
      if (s.id == EMPTY_STUDENT_RECORD.id) continue;
...
void print_student(student_t *s){
    if (s == NULL || s->id == EMPTY_STUDENT_RECORD.id) {
