...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    if (cmd_buff == NULL) return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_MEMORY);

    rsp_buff = malloc(sizeof(char) * RDSH_COMM_BUFF_SZ);
    if (rsp_buff == NULL) return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_MEMORY);
    ...
    while (1) 
    {
      ...
      int err = send(cli_socket, cmd_buff, strlen(cmd_buff) + 1, 0);
      if (err == -1) return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);

      is_eof = false;
      while (!is_eof) {
        io_size = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0);
        if (io_size < 0) return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        if (io_size == 0) return client_cleanup(cli_socket, cmd_buff, rsp_buff, OK);
...
int start_client(char *server_ip, int port){
    ...
    if (cli_socket == -1) return ERR_RDSH_CLIENT;
    ...
    if (ret == -1) return ERR_RDSH_CLIENT;
