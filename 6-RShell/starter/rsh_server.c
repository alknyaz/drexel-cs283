...
int boot_server(char *ifaces, int port){
    ...
    if (svr_socket == -1) return ERR_RDSH_SERVER;
    ...
    if (ret == -1) return ERR_RDSH_SERVER;
...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (cli_socket == -1) return ERR_RDSH_COMMUNICATION;
...
int send_message_string(int cli_socket, char *buff){
    ...
    int io_size = send(cli_socket, buff, send_len, 0);
    if (io_size != send_len) return ERR_RDSH_COMMUNICATION;

    io_size = send(cli_socket, &RDSH_EOF_CHAR, 1, 0);
    if (io_size != 1) return ERR_RDSH_COMMUNICATION;
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    if (bi_cmd == BI_CMD_EXIT) return EXIT_SC;
    if (bi_cmd == BI_CMD_STOP_SVR) return STOP_SERVER_SC;
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == -1) return ERR_RDSH_CMD_EXEC;
        if (pids[i] == 0) {
          if (i == 0) dup2(cli_sock, STDIN_FILENO);
          if (i == clist->num - 1) dup2(cli_sock, STDOUT_FILENO);
